...
test_image_paths = [f for f in listdir('data_test/') if '.jpg' in f]

datas = []
for fname in test_image_paths[:3]:
    file_path = f'./data_test/{fname}'
    start = time.time()
    # print(clothing_image)
    #file_path = f'video_frames/{clothing_image}'
    ...
    write_res = cv2.imwrite(f'test_detectron/{file_name}', result)
...
with open("detectron_outputs.txt", 'w') as t:
    t.write(d)
