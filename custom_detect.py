...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        end_time = time.time()
        # if pred != [None]:
        # data_to_save.append(f'{s} {end_time-start_time}ms')
        # else:
        #    data_to_save.append(f'N/A {end_time-start_time}ms')
        fname = path.split('/')[-1]
        if pred != [None]:
            data_to_save.append(
                # '\n'.join([f'{fname}: {l} {end_time-start_time}s' for l in labels]))
                '\n'.join([f'{fname}: {l} {end_time-start_time}s' + ' %gx%g ' % img.shape[2:] for l in labels]))

        else:
            data_to_save.append(f'{fname}: N/A {end_time-start_time}s')

    with open('output/outputs.txt', 'w') as t:
        t.write('\n'.join(data_to_save))
